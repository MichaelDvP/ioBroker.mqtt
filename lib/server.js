...
function MQTTServer(adapter, states) {
    ...
    function pattern2RegEx(pattern) {
        ...
        if (pattern === '+') return (pattern = '^[^\\.]*$');
        if (pattern.length > 1) {
            if (pattern[pattern.length - 2] !== '.' && pattern[pattern.length - 1] === '#') return (pattern = '');
            for (let p = 1; p < pattern.length; p++) {
                if (pattern[p] === '+' && pattern[p - 1]  !== '.') return (pattern = '');
                if (pattern[p - 1] === '+' && pattern[p]  !== '.') return (pattern = '');
				if (pattern[p - 1] === '#') return (pattern = '');
            }
        }
        pattern = pattern.replace(/\./g, '\\.');
        pattern = pattern.replace(/\\\.#/g, '(\\..*)?');
            ...
            client.on('subscribe', packet => {
                ...
                for (let i = 0; i < packet.subscriptions.length; i++) {
                    ...
                    if (id.indexOf('*') === -1 && id.indexOf('#') === -1 && id.indexOf('+') === -1) {
                        ...
                        if (regText === '') {
                            adapter.log.warn(`Client [${client.id}] subscribe on "${topic}": Invalid topic`);
                            continue;
                        }
                        client._subs[topic] = {
