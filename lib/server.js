    ...
    function startServer(config, socket, server, port, bind, ssl, ws) {
        socket.on('connection', stream => {
            ...
            client.on('publish', async packet => {
                ...
                if (adapter.config.debug) {
                    adapter.log.debug(`Client [${client.id}] received publish ID $(packet.messageId) ${JSON.stringify(packet)}`);
                }
            ...
            client.on('pubrec', packet => {
                ...
                if (adapter.config.debug) {
                    adapter.log.debug(`Client [${client.id}] received pubrec ID $(packet.messageId) ${JSON.stringify(packet)}`);
                }
            ...
            client.on('pubcomp', packet => {
                ...
                if (adapter.config.debug) {
                    adapter.log.debug(`Client [${client.id}] received pubcomp ID $(packet.messageId) ${JSON.stringify(packet)}`);
                }
            ...
            client.on('pubrel', async packet => {
                ...
                if (adapter.config.debug) {
                    adapter.log.debug(`Client [${client.id}] received pubrel ID $(packet.messageId) ${JSON.stringify(packet)}`);
                }
            ...
            client.on('puback', packet => {
                ...
                if (adapter.config.debug) {
                    adapter.log.debug(`Client [${client.id}] received puback ID $(packet.messageId) ${JSON.stringify(packet)}`);
                }
                ...
                for (let m = clients[clientId]._messages.length - 1; m >= 0; m--) {
                    ...
                    if (now - message.ts >= adapter.config.retransmitInterval) {
                        if (message.count > adapter.config.retransmitCount) {
                            ...
                            // adapter.log.warn(`command: ${message.cmd} message topic: ${message.topic}, payload: ${message.payload}`);
                        ...
                        try {
                            if (message.cmd === 'publish') {
                                ...
                                adapter.log.info(`${message.cmd} failed, #${message.messageId} deleted, topic: ${message.topic}, payload: ${message.payload}`);
